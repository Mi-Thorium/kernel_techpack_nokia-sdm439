                                    ...
                                    size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
		if (self_test_buf == NULL)
			self_test_buf = kzalloc(len,GFP_KERNEL);
        ...
        if (val == 0x01)
        {
            ret += snprintf(self_test_buf + ret, len - ret, "Pass\n");
        }
        else
        {
            ret += snprintf(self_test_buf + ret, len - ret, "Fail\n");
        }
        g_self_test_entered = 0;
        if(copy_to_user(buf, self_test_buf, len))
            I("%s,here:%d\n",__func__,__LINE__);
        //kfree(self_test_buf);
        HX_PROC_SEND_FLAG=1;
    }
