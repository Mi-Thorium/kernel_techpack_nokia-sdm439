...
#endif
//add for hxtp Touchscreen to reduce USB influence,wt_fangzhihua,2019-6-27
#if defined(HX_USB_DETECT_GLOBAL) 
extern bool get_upmu_is_chr_det(void);
#endif
//end,wt_fangzhihua
...
#endif
#endif
//end, update for both 89571 and 89626 tp FW, added by fangzhihua.wt,20181126
...
#if 0
    //add for hxtp Touchscreen to reduce USB influence,wt_fangzhihua,2019-6-27
    //connect_status = get_upmu_is_chr_det();         //need add this function  in smb5-lib.c
    //end,wt_fangzhihua,2019-6-27
#else
...
FW_force_upgrade:
    ...
    ts->himax_resume_wq = create_singlethread_workqueue("HMX_RESUME_reuqest");
    if (!ts->himax_resume_wq)
    {
        E(" allocate himax_resume_wq failed\n");
        err = -ENOMEM;
        goto err_himax_resume_wq_failed;
    }
    INIT_WORK(&ts->work_resume, himax_resume_work);
#endif
...
err_alloc_monitor_data:
	...
	destroy_workqueue(ts->himax_resume_wq);
        err_himax_resume_wq_failed:
