...
struct proc_dir_entry *fts_touch_proc_dir;
struct proc_dir_entry *fts_proc_SMWP_file = NULL;



#define FTS_PROC_TOUCH 			"touchscreen"
struct proc_dir_entry *fts_touch_proc_dir;

#define FTS_PROC_TOUCH_LOCKDOWN 	"lockdown_info"

struct proc_dir_entry *fts_proc_lockdown_file = NULL;
...
static ssize_t fts_tplockdown_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	...
	if(*pos) {
		FTS_ERROR("fts_tplockdown_read return\n");
		return 0;
	}
	*pos += len;
...
static struct file_operations fts_proc_lockdown_ops = {
	.owner = THIS_MODULE,
	.read = fts_tplockdown_read,
//	.write = fts_tplockdown_write,
};


void fts_tplockdown_node(void)
{

        fts_touch_proc_dir = proc_mkdir(FTS_PROC_TOUCH, NULL);         //touchscreen

	if (fts_touch_proc_dir == NULL) {
		FTS_ERROR(" %s: fts_touch_proc_dir file create failed!\n", __func__);
	}

	fts_proc_lockdown_file = proc_create(FTS_PROC_TOUCH_LOCKDOWN, (S_IWUSR | S_IRUGO | S_IWUGO),                  //gesture_onoff
									   fts_touch_proc_dir, &fts_proc_lockdown_ops);
	if (fts_proc_lockdown_file == NULL) {
		FTS_ERROR(" %s: proc lockdown file create failed!\n", __func__);
	}
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    fts_tplockdown_node();  //fangzhhua.wt add tp lockdown 20191010
