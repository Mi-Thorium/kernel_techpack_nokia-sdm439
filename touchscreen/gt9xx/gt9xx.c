...
void gtp_int_output(struct goodix_ts_data *ts, int level)
{
	...
	dev_err(&ts->client->dev, "gtp_int_output level = %d\n", level);
				    ...
				    const char *buf, size_t count)
{
	...
	dev_err(&ts->client->dev, "gtp_dofwupdate_store\n");
	...
	dev_err(&ts->client->dev, "gtp_dofwupdate_store retval= %d  count=%d\n",retval,(int)count);
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	if (buf[5] == 0x00) {
		...
		dev_info(&client->dev, "IC Version1: %c%c%c_%02X%02X\n",
			 buf[2], buf[3], buf[4], buf[7], buf[6]);
	} else {
		memcpy(fw_info->pid, buf + GTP_ADDR_LENGTH, 4);
		dev_info(&client->dev, "IC Version2: %c%c%c%c_%02X%02X\n",
