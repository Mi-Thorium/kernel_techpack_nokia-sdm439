...
#define KEY_DOWN_WINGTECH 0x281
#define KEY_UP_WINGTECH 0x280
#define KEY_LEFT_WINGTECH 0x282
#define KEY_RIGHT_WINGTECH 0x283
...
//---------------------------------------------------------------------------------
#define SF_DRV_VERSION "v2.2.30-2018-09-18"
...
static fastscreen_on_data_t *g_fastscreen = NULL;
...
static irqreturn_t sf_ctl_device_irq(int irq, void *dev_id)
{
    ...
    fastscreen_on_data_t *fastscreen_dev = g_fastscreen;
...
static int sf_ctl_report_key_event(struct input_dev *input, sf_key_event_t *kevent)
{
    ...
    switch (kevent->key) {
        ...
        case SF_KEY_UP:
            key_code = KEY_UP_WINGTECH;
            break;

        case SF_KEY_LEFT:
            key_code = KEY_LEFT_WINGTECH;
            break;

        case SF_KEY_RIGHT:
            key_code = KEY_RIGHT_WINGTECH;
            break;

        case SF_KEY_DOWN:
            //key_code = KEY_DOWN;
            key_code = KEY_DOWN_WINGTECH;
            break;
                                   ...
                                   unsigned long event, void *data)
{
    ...
    switch (blank) {
        ...
        case FB_BLANK_POWERDOWN:
            ...
            fastscreen_dev->wait_finger_down = true;
...
static int sf_probe(sf_device_t *dev)
{
    ...
    fastscreen_on_data_t *fastscreen_dev = NULL;
    ...
    fastscreen_dev = kzalloc(sizeof(*fastscreen_dev), GFP_KERNEL);
    if (!fastscreen_dev) {
        err = -ENOMEM;
        printk("%s, Failed to alloc memory for fastscreen_dev device.\n", __func__);
        return err;
    }
    g_fastscreen = fastscreen_dev;

	fastscreen_dev->wait_finger_down = false;
	INIT_WORK(&fastscreen_dev->work, notification_work);
...
static int sf_ctl_init_input(void)
{
    ...
    __set_bit(KEY_UP_WINGTECH,     sf_ctl_dev.input->keybit);
    __set_bit(KEY_LEFT_WINGTECH,   sf_ctl_dev.input->keybit);
    __set_bit(KEY_RIGHT_WINGTECH,  sf_ctl_dev.input->keybit);
    //__set_bit(KEY_DOWN,   sf_ctl_dev.input->keybit);
    __set_bit(KEY_DOWN_WINGTECH,   sf_ctl_dev.input->keybit);    
